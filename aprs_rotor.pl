...
if ($DoAPRSIS) {
	$thr_aprsis = threads->create(
		sub {
			...
			$is->connect('retryuntil' => 3) || die ("\nFailed to connect: $is->{error}\n");

			$SIG{PIPE} = "IGNORE";
			$SIG{'KILL'} = sub { 
				print "\nClosing APRS-IS connection...";
				if ($is->connected()) {
					$is->disconnect() || die "\nFailed to disconnect: $is->{error}\n";
					threads->exit(); 
					};
				};
...
sub SubSTDIN {
	...
	$thr_stdin = threads->create(
	   ...
	   while(my $p = <> ) {
			    ...
			    chomp $p;
				next if ($p eq "");			# skip blank lines
				select undef, undef, undef, 5;	# 5 sec.
				$p = $p . "\n";
				$q->enqueue($p);
	       }
		$q->enqueue(undef);
...
#		$| = 1;			# flush buffer
		...
		if ($retval != 1) {
				...
				next;
		} else {		# decoding ok, do something with the data
